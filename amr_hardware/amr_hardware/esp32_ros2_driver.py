...
class ESP32ROS2Driver(Node):
    def __init__(self):
        ...
        self.declare_parameter('wheel_diameter', 0.065)  # Đường kính bánh xe 65mm (thực tế)
        self.declare_parameter('wheel_base', 0.4722)     # Khoảng cách 2 bánh 472.2mm (từ URDF)
        ...
        # Broadcaster phát TF động odom -> base_footprint
        self.tf_broadcaster = TransformBroadcaster(self)

        # Publishers (đổi tên topic để khớp với ekf_real_robot.yaml)
    ...
    def drive_cb(self, msg: Twist):
        ...
        self.send_ser(f"L{rpm_l:.2f}\n")
        self.send_ser(f"R{rpm_r:.2f}\n")
    ...
    def process_sensor_data(self, ticks_l, ticks_r, gyro_z):
        ...
        # 1. Broadcast TF động: odom -> base_footprint (Cực kỳ quan trọng để nối TF Tree)
        t = TransformStamped()
